...
public class DMetodoPago {
    ...
    public void guardar(String nombre, String descripcion) throws SQLException {
        String query = "INSERT INTO MetodoPago(nombre, descripcion) VALUES(?, ?)";
    ...
    public void modificar(int id, String nombre, String descripcion) throws SQLException {
        String query = "UPDATE MetodoPago SET nombre=?, descripcion=?, actualizado_en=CURRENT_TIMESTAMP WHERE id=?";
        PreparedStatement ps = connection.connect().prepareStatement(query);
        ps.setString(1, nombre);
        ps.setString(2, descripcion);
        ps.setInt(3, id);
    ...
    public void eliminar(int id) throws SQLException {
        String query = "DELETE FROM MetodoPago WHERE id=?";
        PreparedStatement ps = connection.connect().prepareStatement(query);
        ps.setInt(1, id);
        if (ps.executeUpdate() == 0) throw new SQLException();
    }
    ...
    public List<String[]> listar() throws SQLException {
        ...
        String query = "SELECT id, nombre, descripcion FROM MetodoPago ORDER BY id";
        PreparedStatement ps = connection.connect().prepareStatement(query);
    ...
    public int getIdByNombre(String nombre) throws SQLException {
        String query = "SELECT id FROM MetodoPago WHERE nombre=?";
        PreparedStatement ps = connection.connect().prepareStatement(query);
        ps.setString(1, nombre);
        ResultSet rs = ps.executeQuery();
